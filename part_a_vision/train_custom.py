...
class Trainer:
    ...
    def __init__(
        ...
        # Loss: Combined Dice + BCE + Boundary
        ...
        # Optimizer
        is_segformer = "segformer" in model.__class__.__name__.lower() or \
                       (hasattr(model, "model") and "segformer" in model.model.__class__.__name__.lower())
        ...
        if is_segformer:
            ...
            from part_a_config import ENCODER_LR, DECODER_LR
    ...
    def train(self) -> Dict[str, float]:
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            logger.info(
                "Epoch %3d/%d | "
                "Combined=%.4f (Dice=%.4f, BCE=%.4f, Boundary=%.4f, Conn=%.4f, Aux=%.4f) train_iou=%.3f | "
                "val_loss=%.4f val_iou=%.3f | "
                "lr=%.2e | %.1fs %s",
                epoch, self.epochs,
                train_loss, train_metrics.get("loss_dice", 0), train_metrics.get("loss_bce", 0), train_metrics.get("loss_boundary", 0), train_metrics.get("loss_conn", 0), train_metrics.get("loss_aux", 0), train_metrics.get("iou", 0),
    ...
    def _train_one_epoch(self) -> Tuple[float, Dict[str, float]]:
        ...
        for batch_idx, (fused, mask) in enumerate(self.train_loader):
            ...
            with autocast(enabled=self.use_amp):
                # Phase 13: check if model supports return_gate
                if "return_gate" in self.model.forward.__code__.co_varnames:
                    output, gate_weights, cloud_frac = self.model(fused, return_gate=True)
                else:
                ...
                if isinstance(output, dict):
                    output = output["out"]
                loss, comps = self.criterion(output, mask)
                
                if gate_weights is not None and cloud_frac is not None:
                    import torch.nn.functional as F
                    w_opt, w_sar = gate_weights
                    # Aux loss: w_sar should match cloud fraction (0% cloud -> w_sar ~0, 100% cloud -> w_sar ~1)
                    aux_loss = F.mse_loss(w_sar.mean(dim=(2, 3)), cloud_frac)
                    loss = loss + aux_loss
                    comps["loss_aux"] = aux_loss.item()
            ...
            running_dice += comps.get('dice', 0.0)
            running_bce += comps.get('bce', 0.0)
            running_boundary += comps.get('boundary', 0.0)
            running_conn += comps.get('conn', 0.0)
            running_aux += comps.get('loss_aux', 0.0)
        ...
        avg_metrics["loss_dice"] = running_dice / n_batches
...
def main():
    parser = argparse.ArgumentParser(
        description="Phase 7/11 - Baseline/SegFormer Training on Synthetic Tiles"
    )
    ...
    # Build model
    model = build_model(backbone=args.backbone, input_channels=INPUT_CHANNELS, num_classes=NUM_CLASSES)
    logger.info("Model Backbone: %s, %d input channels", args.backbone, INPUT_CHANNELS)
